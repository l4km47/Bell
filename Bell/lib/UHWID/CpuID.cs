...
namespace RcisSchoolBell.lib.UHWID
{
    public static class CpuId
    {
        ...
        private static bool ExecuteCode(ref byte[] result)
        {
            ...
            if (IsX64Process())
            {
                var ptr = new IntPtr(codeX64.Length);

                if (!VirtualProtect(codeX64, ptr, PageExecuteReadwrite, out num))
                    Marshal.ThrowExceptionForHR(Marshal.GetHRForLastWin32Error());

                ptr = new IntPtr(result.Length);

                return (CallWindowProcW(codeX64, IntPtr.Zero, 0, result, ptr) != IntPtr.Zero);
            }
            else
            {
                var ptr = new IntPtr(codeX86.Length);

                if (!VirtualProtect(codeX86, ptr, PageExecuteReadwrite, out num))
                    Marshal.ThrowExceptionForHR(Marshal.GetHRForLastWin32Error());

                ptr = new IntPtr(result.Length);
                return (CallWindowProcW(codeX86, IntPtr.Zero, 0, result, ptr) != IntPtr.Zero);
            }
